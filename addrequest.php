...
$id=$_POST['id'];
	...
	$busca=mysqli_query($mysqli,"SELECT * FROM pedidos where id='$id'");
	$cuantos=mysqli_num_rows($busca);
	//El programa se detendra una vez encuentre un id repetido
	if($cuantos>0){ 
	echo("Este pedido ya fue registrado, no es necesario registrarlo de nuevo <br>");
	?>
	<a href="index.php?action=addrequest">Volver</a>
	<?php
	exit();
	}
	//En caso contrario continua la operacion
	...
	if($add=mysqli_query($mysqli,"INSERT into pedidos(id,solicitante,fecha_registro,destino,monto,plazo) values('$id','$solicitante','$fecharegistro','$destino','$monto','$plazo') ")){ 

		echo("Se han registrado los datos del pedido, gracias por usar el servicio <br>");
	}
?>
	<a href="index.php?action=addrequest">Volver</a>